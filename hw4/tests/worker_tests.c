...
extern int read_data_t(FILE *in, char *resultp, int nbytes);
extern int write_data_t(FILE *out, char *datap, int nbytes);
 ...
 */
static void killall() {
  system("killall -q -s KILL polya_worker");
  system("killall -q -s KILL bin/polya");
}
...
static int write_problem(FILE *out, struct problem *prob) {
    ...
    // Write the header.
    if(write_data_t(out, (char *)prob, sizeof(*prob)) == EOF)
	return EOF;
...
static struct result *read_result(FILE *in) {
    ...
    if(read_data_t(in, (char *)&rhdr, sizeof(*result)) == EOF)
	return NULL;
...
/*
 * worker_test_5
 * Ensure a worker process can correctly solve a crypto miner problem type
 */
Test(worker_suite, worker_test_5, .timeout=60, .fini = killall) {
    FILE *from_worker = NULL, *to_worker = NULL;
    pid_t pid = launch_worker(&from_worker, &to_worker);
    ...
    struct result *result = read_result(from_worker);
    cr_assert_not_null(result, "Failed reading result from worker.\n");
    cr_assert_eq(result->failed, 0, "Worker returned a 'failed' result for a crypto miner problem.\n");
...
Test(worker_suite, worker_test_series_of_problems, .timeout=60, .fini = killall) {
    FILE *from_worker = NULL, *to_worker = NULL;
    pid_t pid = launch_worker(&from_worker, &to_worker);
...
/* A crypto miner problem for the worker to solve */
    ...
    size_t bsize = 32, nsize = 8;
    size_t size = sizeof(struct crypto_miner_problem) + bsize + nsize;
    struct crypto_miner_problem *prob_cm = malloc(size);
    prob_cm->size = size;
    prob_cm->type = CRYPTO_MINER_PROBLEM_TYPE;
    prob_cm->id = 5;
    prob_cm->nvars = 1;
    prob_cm->var = 0;
    prob_cm->bsize = bsize;
    prob_cm->nsize = nsize;
    prob_cm->diff = 25;
    memcpy(prob_cm->data, data, bsize+nsize);
    ...
    /* Send the problem to the worker */
    write_problem(to_worker, (struct problem *) prob_cm);

    /* Wait for the worker to SIGSTOP itself again */
    Waitpid(pid, &status, WUNTRACED);

    /* Read and validate the problem's result, if one was given */
    struct result *result_cm = read_result(from_worker);
    cr_assert_not_null(result_cm, "Failed reading result from worker.\n");
    cr_assert_eq(result_cm->failed, 0, "Worker returned a 'failed' result for a crypto miner problem.\n");

    /* Validate that the returned result struct is byte-for-byte the one we expect */
    unsigned char expected_result_cm[] = {0x20, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8, 0x0, 0x0, 0x0, 0x8, 0xef, 0xa9, 0xc0, 0xc2, 0xb3, 0xc3, 0x98, 0x0, 0x0, 0x0, 0x0};
    cr_assert_eq(result_cm->size, sizeof(expected_result_cm), "Returned struct was of size %d (expected %d)\n", result_cm->size, sizeof(expected_result_cm));
    unsigned char *current_byte = (unsigned char *) result_cm;
    for (int i=0; i<sizeof(expected_result_cm); i++) {
        cr_assert_eq(*current_byte, expected_result_cm[i], "Returned result differed from expected result (expected %x, but found %x at byte %d out of %d)!", expected_result_cm[i], *current_byte, i, sizeof(expected_result_cm));
        current_byte++;
    }
    ...
    /* Read and validate the problem's result, if one was given */
    struct result *result2 = read_result(from_worker);
    cr_assert_not_null(result2, "Failed reading result from worker.\n");
    cr_assert_eq(result2->failed, 0, "Worker returned a 'failed' result for a trivial problem.\n");
}
