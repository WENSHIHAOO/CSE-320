...
int run_test(char *name, char *target, char *av[], COMMAND *script, int *statusp) {
    ...
    // Array to track job IDs returned in CREATE events.
    int jobids[JOBIDS_MAX];
    int nextid = 0;
	...
	if(scrp->send) {
	    // Send next command to child process, handling substitution of job IDs.
	    char buf[SEND_MAX];
	    fprintf(stderr, ">>> %s\n", buf);
	    fprintf(cmd_str, "%s\n", buf);
	    fflush(cmd_str);
	...
	if(scrp->expect) {
		...
		scrp->before(ep, jobids, scrp->args);
		...
		scrp->after(ep, jobids, scrp->args);
	      if(ep == NULL) {
		...
		if(ep->type == CHANGE_EVENT && ep->newstate == WORKER_STARTED) {
		    fprintf(stderr, "Job $%d => ID %d\n", nextid, ep->pid);
		    jobids[nextid++] = ep->pid;
		}
		break;
...
static FILE *run_target(char *prog, char *av[], int *pidp) {
	...
	rl.rlim_cur = rl.rlim_max = 500;  // 500 processes maximum
...
static EVENT *read_event(int fd, struct timeval *timeout) {
	...
	if(n < 0) {
	    ...
	    fprintf(stderr, "EOF reading event from child\n");
	}
	return NULL;
    }
    setitimer(ITIMER_REAL, &itv, NULL);  // Cancel timer
