...
Test(demo_master_suite, startup_test) {
    char *cmd = "timeout -s KILL 5s demo/polya";
...
Test(demo_master_suite, trivial_test) {
    char *cmd = "timeout -s KILL 5s demo/polya -p 1 -t 1";
    int return_code = WEXITSTATUS(system(cmd));
...
Test(demo_master_suite, miner_test_one_worker) {
    char *cmd = "timeout -s KILL 120s demo/polya -p 5 -t 2";
    int return_code = WEXITSTATUS(system(cmd));
...
Test(demo_master_suite, miner_test_three_workers) {
    char *cmd = "timeout -s KILL 60s demo/polya -p 5 -t 2 -w 3";
    int return_code = WEXITSTATUS(system(cmd));
